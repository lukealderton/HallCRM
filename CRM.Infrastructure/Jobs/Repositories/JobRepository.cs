    ...
    CancellationToken objToken = default)
        {
            ...
            {
                objQuery = objQuery.Where(objJob =>
                    objJob.Stage == enmStage.Value);
            }

            if (objCompanyId.HasValue)
            {
                objQuery = objQuery.Where(objJob =>
                    objJob.CompanyId == objCompanyId.Value);
            }
            ...
            if (blnOverdueOnly)
            {
                ...
                objQuery = objQuery.Where(objJob =>
                    ...
                    objJob.ExpectedCloseDateUtc.Value < dteTodayUtc);
            }
            ...
            if (!String.IsNullOrWhiteSpace(strSearch))
            {
                ...
                objQuery = objQuery.Where(objJob =>
                    ...
                    (
                        ...
                        objJob.Description.Contains(strKeyword)
                    ) ||
                    (
                        objJob.Source != null &&
                        objJob.Source.Contains(strKeyword)
                    ) ||
                    (
                        objJob.Company != null &&
                        objJob.Company.Name.Contains(strKeyword)
                    ) ||
                    (
                        objJob.Contact != null &&
                        objJob.Contact.Entity.DisplayName.Contains(strKeyword)
                    ));
            }

            return await objQuery
                .OrderByDescending(objJob =>
                    objJob.ExpectedCloseDateUtc.HasValue)
                .ThenBy(objJob =>
